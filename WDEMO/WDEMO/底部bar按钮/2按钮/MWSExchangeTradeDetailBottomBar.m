//
//  MWSExchangeTradeDetailBottomBar.m
//  MoWanShang
//
//  Created by aimeiju on 2022/6/30.
//

#import "MWSExchangeTradeDetailBottomBar.h"

@implementation MWSExchangeTradeDetailBottomBar
...
//    self.view.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.6];
    ...
    [self.detailBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.width.equalTo(@(KWidth / 2.0 - 12 * 2));
    }];
...
- (UIButton *)detailBtn {
    if (!_detailBtn) {
        ...
        [_detailBtn setTitle:@"关闭交易" forState:UIControlStateNormal];
        [_detailBtn setBackgroundImage:[UIImage imageNamed:@"btn_blackline"] forState:UIControlStateNormal];
        _detailBtn.titleEdgeInsets = UIEdgeInsetsMake(-6, 0, 0, 0);
...
- (UIButton *)nextBtn {
    if (!_nextBtn) {
        ...
        [_nextBtn setTitle:@"选择换物" forState:UIControlStateNormal];
        [_nextBtn setBackgroundImage:[UIImage imageNamed:@"btn_redline"] forState:UIControlStateNormal];
        _nextBtn.titleEdgeInsets = UIEdgeInsetsMake(-6, 0, 0, 0);
