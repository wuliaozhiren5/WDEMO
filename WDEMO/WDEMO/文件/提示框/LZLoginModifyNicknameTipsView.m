//
//  LZLoginModifyNicknameTipsView.m
//  longzhu
//
...
@interface LZLoginModifyNicknameTipsView ()
...
@implementation LZLoginModifyNicknameTipsView
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [_confirmBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(_contentLab.mas_bottom).offset(26);
//            make.centerX.equalTo(_container);
            make.width.equalTo(@125);
            make.height.equalTo(@42);
            make.trailing.equalTo(_container.mas_centerX).offset(-5);
        }];
        
        _editBtn = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, 260, 42)];
        _editBtn.layer.cornerRadius = 4;
        _editBtn.layer.masksToBounds = YES;
        _editBtn.backgroundColor = [UIColor orangeColor];
        _editBtn.titleLabel.font = [UIFont boldSystemFontOfSize:16.0];
        [_editBtn setTitle:@"立即修改" forState:UIControlStateNormal];
        [_editBtn setTitleColor:[UIColor colorFromHexRGB:@"ffffff"] forState:UIControlStateNormal];
        [_editBtn addTarget:self action:@selector(clickEditBtn) forControlEvents:UIControlEventTouchUpInside];
        [_container addSubview:_editBtn];
        [_editBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(_contentLab.mas_bottom).offset(26);
            //            make.centerX.equalTo(_container);
            make.width.equalTo(@125);
            make.height.equalTo(@42);
            make.leading.equalTo(_container.mas_centerX).offset(5);
        }];
